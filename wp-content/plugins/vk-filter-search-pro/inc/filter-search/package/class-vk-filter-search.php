...
if ( ! class_exists( 'VK_Filter_Search' ) ) {
	...
	class VK_Filter_Search {
		...
		public static function get_taxonomy_form_html( $taxonomy, $options = array() ) {
			...
			if ( ! empty( $taxonomy_object ) && ! empty( $taxonomy_terms ) ) {
				...
				if ( 'user' !== $options['operator'] ) {
					...
					$taxonomy_form_html .= __( 'AND Search', 'vk-filter-search-pro' );
					$taxonomy_form_html .= '</label></li>';
					$taxonomy_form_html .= '<li class="vkfs_prefix__level-0"><label>';
					$taxonomy_form_html .= '<input type="radio" name="vkfs_' . $taxonomy . '_operator" value="or">';
					$taxonomy_form_html .= __( 'OR Search', 'vk-filter-search-pro' );
					$taxonomy_form_html .= '</label></li>';
		...
		public static function get_search_count_html( $options ) {
			$default = array(
				'outer'             => true,
				'before_text'       => __( 'Search Results:', 'vk-filter-search-pro' ),
				'after_text'        => __( 'articles found.', 'vk-filter-search-pro' ),
				'number_color'      => '',
				'number_font_size'  => '',
				'number_font_style' => '',
				'number_font_weight' => '',
		...
		public static function form_edit_content( $post_id, $old_flag ) {
			...
			if ( current_user_can( 'edit_pages' ) ) {
				if ( true === $old_flag ) {
					...
					$edit_content .= __( 'This form is obsolete and may eventually be deleted.', 'vk-filter-search-pro' );
					$edit_content .= '<br>';
					$edit_content .= __( 'We recommend creating a form with the post type "VK Filteer Search".', 'vk-filter-search-pro' );
					$edit_content .= '<br>';
					$edit_content .= __( 'The post of "VK Filter Search" can be called by the "Call Filter Search" block.', 'vk-filter-search-pro' );
					$edit_content .= '</div>';
					$edit_content .= '</div>';
				}
				$edit_content .= '<a class="btn btn-default btn-sm" href="' . get_edit_post_link( $post_id ) . '" target="_blank">';
				$edit_content .= __( 'Edit', 'vk-filter-search-pro' );
				$edit_content .= '</a>';
			}
			return $edit_content;
		}
