	...
	class VK_Filter_Search_Title {
		...
		public static function get_default_search_title_args() {
			$search_title_args = array(
				'queries_format'          => __( 'Search Result for %s', 'vk-filter-search-pro' ),
				...
				'query_date_min_format'   => __( 'From %s', 'vk-filter-search-pro' ),
				'query_date_max_format'   => __( 'To %s', 'vk-filter-search-pro' ),
				'query_date_range_format' => __( 'From %1$s to %2$s', 'vk-filter-search-pro' ),
		...
		public static function get_post_type_search_title( $search_title_args ) {
			...
			if ( ! empty( $post_type_value ) && $post_type_value !== 'any' ) {
				...
				if ( 'display' === $search_title_args['query_title_display'] ) {
					$search_title .= __( 'Post Type', 'vk-filter-search-pro' ) . $search_title_args['query_title_after'];
				}
		...
		public static function get_keyword_search_title( $search_title_args ) {
			...
			if ( ! empty( $keyword_array ) ) {
				// query_title_display が display の場合はクエリタイトルを表示
				if ( 'display' === $search_title_args['query_title_display'] ) {
					$search_title .= __( 'Keyword', 'vk-filter-search-pro' ) . $search_title_args['query_title_after'];
				}
				$count = 0;
