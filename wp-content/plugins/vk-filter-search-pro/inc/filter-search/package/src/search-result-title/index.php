...
function vkfs_search_result_title_render_callback( $attributes, $content ) {
	$attributes = wp_parse_args(
		...
		array(
			'outerTagName'          => 'div',
			'queriesFormat'         => __( 'Search Result for %s', 'vk-filter-search-pro' ),
			...
			'queryElementAfter'     => '"',
			'queryElementsAfter'    => ' , ',
			'queryDateMinFormat'    => __( 'From %s', 'vk-filter-search-pro' ),
			'queryDateMaxFormat'    => __( 'To %s', 'vk-filter-search-pro' ),
			'queryDaterRangeFormat' => __( 'From %1$s to %2$s', 'vk-filter-search-pro' ),
		)
	);
	...
	if ( is_search() ) {
		...
		$options = array(
			'queries_format'          => ! empty( $attributes['queriesFormat'] ) ? $attributes['queriesFormat'] : __( 'Search Result for %s', 'vk-filter-search-pro' ),
			...
			'query_date_min_format'   => ! empty( $attributes['queryDateMinFormat'] ) ? $attributes['queryDateMinFormat'] : __( 'From %s', 'vk-filter-search-pro' ),
			'query_date_max_format'   => ! empty( $attributes['queryDateMaxFormat'] ) ? $attributes['queryDateMaxFormat'] : __( 'To %s', 'vk-filter-search-pro' ),
			'query_date_range_format' => ! empty( $attributes['queryDaterRangeFormat'] ) ? $attributes['queryDaterRangeFormat'] : __( 'From %1$s to %2$s', 'vk-filter-search-pro' ),
			'queries_after'           => '',
		);
