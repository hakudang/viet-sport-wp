...
function vkfs_call_filteer_search_render_callback( $attributes ) {
	...
	if ( -1 !== $target_id ) {
		...
		if ( $url ) {
			$page_html .= '<button type="button" onClick="window.open(\'' . esc_url( $url ) . '\', \'_blank\', \'noopener,noreferrer\')" class="vkfs__call-filter-search_editBtn btn btn-outline-primary btn-sm veu_adminEdit">' . __( 'Edit Search Form', 'vk-filter-search-pro' ) . '</button>';
		}
		$page_html .= '</div>';
	}
