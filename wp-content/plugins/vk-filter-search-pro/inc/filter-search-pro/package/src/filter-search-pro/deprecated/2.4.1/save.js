...
export default function save( props ) {
	...
	return (
		...
		>
			...
			{ layoutMethod === 'minimum' && layoutBaseWidthMin && (
				<style type="text/css">{ `
				.vkfs__block-id--${ blockID } .vkfs__labels{
					gap: ${ layoutGap };
				}
				.vkfs.vkfs__block-id--${ blockID } button[type=submit] {
					margin-top: calc( ${ layoutGap } + 0.5em );
				}
				.vkfs__block-id--${ blockID } .vkfs__outer-wrap {
					flex-basis: ${ layoutBaseWidthMin };
					min-width: ${ layoutBaseWidthMin };
				}` }</style>
			) }
		</form>
	);
